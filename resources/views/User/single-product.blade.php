<x-main>
    ...
    <div class="pt-12 pb-20 lg:pb-44 lg:pt-24 xl:pt-[105px]">
      ...
      <form class="grid grid-cols-1 md:grid-cols-2 gap-7" action="{{ route('store.review') }}" method="POST">
    @csrf
    <div class="md:col-span-1">
        <label class="block text-sm font-bold uppercase tracking-tight text-gray-900 dark:text-white [&:not(:empty)]:mb-2.5" for="review-title">Review Title</label>
        <input class="group-[.is-success]:bg-input-success-dark group-[.is-error]:border-danger group-[.is-error]:bg-input-invalid group-[.is-error]:text-danger group-[.is-success]:pr-16 group-[.is-invalid]:pr-16 px-4 py-2 border-base bg-white bg-[length:18px_18px,_38px_38px] bg-[position:right_16px_center,_right_6px_center] bg-no-repeat font-medium leading-8 tracking-tight text-gray-900 transition-all duration-150 placeholder:font-normal placeholder:text-gray-500/60 focus:border-accent focus:bg-white focus:text-gray-900 focus:outline-0 focus:ring-0 dark:border-gray-600 dark:group-[.is-success]:bg-input-success dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500/80 dark:focus:border-accent dark:focus:bg-gray-900 w-full" type="text" name="review-title" id="review-title" value="" placeholder="">
        <p class="group-[.is-error]:block mt-2 hidden text-xs text-danger"></p>
    </div>
    <div class="md:col-span-1">
        <label class="block text-sm font-bold uppercase tracking-tight text-gray-900 dark:text-white [&:not(:empty)]:mb-2.5" for="review-rating">Your Rating</label>
        <div class="rating-stars">
            <select class="w-full border-base px-4 tracking-tight text-gray-900 transition-all duration-150 bg-white bg-caret bg-[length:10px_6px] bg-[position:right_24px_center] focus:border-accent focus:outline-0 focus:ring-0 hover:cursor-pointer dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500/80 dark:focus:border-accent dark:bg-caret-white h-[50px]" name="review-rating" id="review-rating">
    ...
    <div class="col-span-full">
        <label class="block text-sm font-bold uppercase tracking-tight text-gray-900 dark:text-white [&:not(:empty)]:mb-2.5" for="review-comment">Your Review</label>
        <textarea class="block border-base bg-white w-full px-4 py-2 tracking-tight text-gray-900 transition-all duration-150 placeholder:text-gray-500/60 focus:border-accent focus:outline-0 focus:ring-0 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500/80 dark:focus:border-accent dark:focus:bg-gray-900 h-32 md:h-40" name="review-comment" id="review-comment" placeholder=""></textarea>
    </div>

    <div class="col-span-full">
        <button type="submit" class="inline-flex text-center font-bold leading-none transition-colors uppercase justify-center gap-x-3 py-4 px-4 md:py-[18px] lg:px-8 text-sm text-white bg-primary hover:bg-primary/90 w-full">Post Review</button>
    </div>
</form>
    ...
    <li class="mb-12 lg:mb-20">
        <div class="flex gap-x-7 md:gap-x-9">
            <figure class="shrink-0 mt-0" style="width: 19%;">
                ...
                <img src="{{ asset($review->user->image)}}" alt="Comment Author Avatar">

            </figure>
            <div class="flex-grow">
                <div class="mb-6">
                    <h5 class="mb-2 text-lg md:text-1.5xl md:mb-3 font-bold leading-none tracking-tighter text-gray-900 dark:text-white uppercase">
                        {{ $review->title }}
                    </h5>

                    <div class="flex items-center gap-x-4 relative text-sm leading-none">
                        @for ($i = 0; $i < $review->rating; $i++)
                            <svg role="img" class="h-[13px] w-[13px] fill-gray-200 dark:fill-white/20">
                                <use xlink:href="assets/img/main/sprite.svg#star"></use>
                            </svg>
                        @endfor
                        @for ($i = $review->rating; $i < 5; $i++)
                            <svg role="img" class="h-[13px] w-[13px] fill-gray-900 dark:fill-white">
                                <use xlink:href="assets/img/main/sprite.svg#star"></use>
                            </svg>
                        @endfor
                    </div>
                ...
                <div class="text-base leading-tight uppercase font-bold tracking-tighter text-gray-900 dark:text-white">
                    by <strong class="text-primary">{{ $review->name }}</strong> <time class="" datetime="{{ $review->created_at }}">{{ $review->created_at->diffForHumans() }}</time>
                </div>
            </div>
        </div>
    </li>
